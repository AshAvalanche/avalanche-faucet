...
import { SendTokenResponse, ChainType, EVMInstanceAndConfig } from "./types";
...
const separateConfigFields = [
  "COUPON_REQUIRED",
  "MAINNET_BALANCE_CHECK_ENABLED",
];
...
router.post(
  "/sendToken",
  typeof process.env.CAPTCHA_SECRET === "string" &&
    process.env.CAPTCHA_SECRET.trim().length == 0
    ? []
    : captcha.middleware,
  async (req: any, res: any) => {
    ...
    const dripAmount =
      erc20Instance?.config.DRIP_AMOUNT ?? evm.config.DRIP_AMOUNT;
     ...
     */
    const mainnetCheckEnabled =
      (erc20Instance
        ? erc20Instance.config.MAINNET_BALANCE_CHECK_ENABLED
        : evm.config.MAINNET_BALANCE_CHECK_ENABLED) ?? false;
    const couponCheckEnabled =
      couponConfig.IS_ENABLED &&
      ((erc20Instance
        ? erc20Instance.config.COUPON_REQUIRED
        : evm.config.COUPON_REQUIRED) ??
        false);

    let pipelineValidity: PipelineCheckValidity = {
      isValid: false,
      dripAmount,
    };
    !pipelineValidity.isValid &&
      couponCheckEnabled &&
      (await checkCouponPipeline(
        couponService,
        pipelineValidity,
        faucetConfigId,
        coupon
    ...
    // send request
    evm.instance.sendToken(
      address,
      erc20,
      pipelineValidity.dripAmount,
      async (data: SendTokenResponse) => {
        const { status, message, txHash } = data;

        // reclaim coupon if transaction is failed
        if (
          pipelineValidity.checkPassedType === PIPELINE_CHECKS.COUPON &&
          coupon &&
          txHash === undefined
        ) {
          await couponService.reclaimCouponAmount(
            coupon,
            pipelineValidity.dripAmount
          );
        }
...
app.get("*", async (req: any, res: any) => {
  ...
  if (NATIVE_CLIENT) {
    ...
    res.redirect(
      `https://core.app/tools/testnet-faucet${chain ? "?subnet=" + chain + (erc20 ? "&token=" + erc20 : "") : ""}`
    );
  }
});
